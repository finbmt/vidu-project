...
function dpPopulate(data) {
	...
	try {
		...
		//the reason only for ssl  limitiation derived from the fact that  the ssl static domain response contains p3p header which allows write cookies in IE .  
	...
	if (location.href.toLowerCase().indexOf("compliance_popup.html") == -1) {
		(function () {
			...
			//var pname = dpVariables.Affiliate.getPoweredByElemV2.textnode !== "" ? dpVariables.Affiliate.getPoweredByElemV2.textnode : dpVariables.Affiliate.broughtByTextForAffiliate;
